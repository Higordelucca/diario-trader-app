...
# A 'chave' é o nome que aparecerá no menu, e o 'valor' é o multiplicador financeiro.
VALORES_POR_PONTO = {
    # --- Ações ---
    "Ação (BRL ou USD)": 1.0,
    # --- Micros Americanos ---
    ...
    # --- Ouro ---
    "Micro Ouro (MGC)": 1.0, # $1.00 por tick de $0.10
    "Ouro (GC)": 10.0,      # $10.00 por tick de $0.10
}
...
class TradingJournalApp:
    def __init__(self, root):
        ...
        notebook.add(self.tab1, text="Registro de Trades")
        ...
        risk_frame = ttk.Labelframe(self.tab1, text="Gerenciamento de Risco", padding=10)
        risk_frame.pack(fill=X, side=TOP, padx=10, pady=5)

        # --- Widgets da Calculadora de Risco ---
        ttk.Label(risk_frame, text="Patrimônio Total ($):").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.patrimonio_entry = ttk.Entry(risk_frame, width=15)
        self.patrimonio_entry.grid(row=0, column=1, padx=5, pady=5)

        usar_saldo_btn = ttk.Button(risk_frame, text="Usar Saldo", command=self._preencher_patrimonio_com_saldo, bootstyle="outline-secondary")
        usar_saldo_btn.grid(row=0, column=2, padx=5, pady=5)

        ttk.Label(risk_frame, text="Risco / Operação (%):").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        self.risco_percent_entry = ttk.Entry(risk_frame, width=15)
        self.risco_percent_entry.grid(row=1, column=1, padx=5, pady=5)
        self.risco_percent_entry.insert(0, "1.0") # Valor padrão de 1%

        calcular_risco_btn = ttk.Button(risk_frame, text="Calcular", command=self._calcular_risco, bootstyle="info")
        calcular_risco_btn.grid(row=0, column=3, rowspan=2, padx=15, pady=5, sticky="ns")

        ttk.Label(risk_frame, text="Perda Máxima ($):", font=("-size 10 -weight bold")).grid(row=0, column=4, padx=10, pady=5, sticky="w")
        self.perda_maxima_label = ttk.Label(risk_frame, text="$ 0.00", font=("-size 14 -weight bold"), bootstyle="danger")
        self.perda_maxima_label.grid(row=1, column=4, padx=10, pady=5, sticky="w")

        # --- Continuação da Calculadora: Position Sizing Inteligente ---
        ttk.Label(risk_frame, text="Preço de Entrada:").grid(row=2, column=0, padx=5, pady=5, sticky="w")
        self.entrada_entry = ttk.Entry(risk_frame, width=15)
        self.entrada_entry.grid(row=2, column=1, padx=5, pady=5)

        ttk.Label(risk_frame, text="Preço do Stop Loss:").grid(row=2, column=2, padx=5, pady=5, sticky="w")
        self.stop_entry = ttk.Entry(risk_frame, width=15)
        self.stop_entry.grid(row=2, column=3, padx=5, pady=5)

        # Widgets modificados/adicionados
        ttk.Label(risk_frame, text="Selecionar Ativo:").grid(row=3, column=0, padx=5, pady=5, sticky="w")
        self.ativo_calculadora_combo = ttk.Combobox(risk_frame, values=list(VALORES_POR_PONTO.keys()), state="readonly")
        self.ativo_calculadora_combo.grid(row=3, column=1, padx=5, pady=5, sticky="ew")
        self.ativo_calculadora_combo.bind("<<ComboboxSelected>>", self._on_asset_select)

        ttk.Label(risk_frame, text="Valor do Ponto ($):").grid(row=3, column=2, padx=5, pady=5, sticky="w")
        self.valor_ponto_entry = ttk.Entry(risk_frame, width=15, state="readonly")
        self.valor_ponto_entry.grid(row=3, column=3, padx=5, pady=5)

        ttk.Label(risk_frame, text="Posição Máxima:", font=("-size 10 -weight bold")).grid(row=2, column=4, padx=10, pady=5, sticky="w")
        self.posicao_maxima_label = ttk.Label(risk_frame, text="0", font=("-size 14 -weight bold"), bootstyle="success")
        self.posicao_maxima_label.grid(row=3, column=4, padx=10, pady=5, sticky="w")
    ...
    def _criar_dashboard(self, parent_frame):
        ...
        ttk.Label(parent_frame, text="PAYOFF RATIO", font=("-size 10")).grid(row=0, column=4, padx=20, pady=(5,0))
        # Label para o valor, que será atualizado
        self.payoff_label = ttk.Label(parent_frame, text="0.00", font=("-size 14 -weight bold"), bootstyle="info")
        self.payoff_label.grid(row=1, column=4, padx=20, pady=(0,10))

        # E ajuste a linha do grid_columnconfigure para incluir a nova coluna 4
    ...
    def _atualizar_dashboard(self, trades):
        ...
        self.saldo_atual = saldo_usd # Guarda o saldo para ser usado pela calculadora
    ...
    def atualizar_dados(self, filtro_ativo=None, data_inicio=None, data_fim=None):
        trades = data_manager.carregar_trades(filtro_ativo=filtro_ativo, data_inicio=data_inicio, data_fim=data_fim); self._popular_tabela(trades); self._atualizar_dashboard(trades); self._atualizar_grafico(trades); 
    ...
    def _calcular_risco(self):
        try:
            # Limpa os resultados antigos para evitar confusão
            ...
            risco_percent = float(self.risco_percent_entry.get())
            ...
            preco_entrada_str = self.entrada_entry.get()
            preco_stop_str = self.stop_entry.get()
            valor_ponto_str = self.valor_ponto_entry.get()

            # Só calcula a posição se todos os campos necessários estiverem preenchidos
            if preco_entrada_str and preco_stop_str and valor_ponto_str:
                ...
                if risco_financeiro_unitario > 0:
                    ...
                    self.posicao_maxima_label.config(text=f"{posicao_maxima} contratos/ações")
                else:
                    self.posicao_maxima_label.config(text="Verifique os preços")

        except (ValueError, TypeError):
            # Lida com o caso de o usuário digitar texto em vez de números
    ...
    def _on_asset_select(self, event):
        ...
        # Habilita, altera e desabilita o campo novamente
        ...
        # Tenta calcular o risco automaticamente para melhorar a experiência
        self._calcular_risco()
    ...
    def _atualizar_grafico(self, trades):
        ...
        # adiciona um linha horizontal no ponto zero
